...
def login_usuario(login, senha):
    with closing(conectar()) as con, closing(con.cursor()) as cur:
        cur.execute("SELECT login, senha, saldo FROM usuarios WHERE login = '" + login + "' AND senha = '" + senha + "'")
        t = cur.fetchone()
        if t is None: return None
        return {"login": t[0], "senha": t[1], "saldo": t[2]}
...
def transferir_valor(de, para, valor):
    sql = f"""
        UPDATE usuarios SET saldo = saldo - {valor} WHERE login = '{de}';
        UPDATE usuarios SET saldo = saldo + {valor} WHERE login = '{para}';
    """
    with closing(conectar()) as con, closing(con.cursor()) as cur:
